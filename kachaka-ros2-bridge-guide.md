...
- **Linux**: そのまま進めばOK
- **macOS (Docker Desktop)**: Docker Desktop の設定で「Host networking」を有効化（Settings → Resources → Network → "Enable host networking"）。kachaka と通信する際は **CycloneDDS** を使うこと（後述）。
- **Windows**: WSL2 上の Ubuntu からの利用を推奨
...
**ブリッジを一度止めて (Ctrl+C)、CycloneDDS で再起動**:

```sh
RMW_IMPLEMENTATION=rmw_cyclonedds_cpp ./start_bridge.sh <kachakaのIP>
```

**別ターミナルで、CycloneDDS のクライアントコンテナを起動**:
